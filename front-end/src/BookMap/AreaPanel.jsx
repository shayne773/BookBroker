...
const areaUrl = (place, offset) =>
  `${import.meta.env.VITE_SERVER_ADDRESS}/map/area?place=${encodeURIComponent(place)}` +
  (offset ? `&offset=${offset}` : '');

const fetchPage = async (place, offset) => {
  const res = await authFetch(areaUrl(place, offset));
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
};

// The side panel for one place on the map: its books on the market, newest
// first, a page at a time. Keyed by place, so a new place starts afresh.
const AreaPanel = ({ place, count, onClose }) => {
  ...
  }, [place, addPage, failed]);

  const showMore = () => {
    setLoading(true);
    fetchPage(place, nextOffset)
      .then(addPage)
      .catch(failed)
  ...
  return (
    <aside className="map-panel" aria-labelledby="map-panel-title">
      ...
      {!loading && !error && books.length === 0 && (
        <p className="empty">No books here right now.</p>
      )}
